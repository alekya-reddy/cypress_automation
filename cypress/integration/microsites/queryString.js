...
const microsite = {
    name: "micrositeQueryString.js",
    slug: "micrositequerystring-js",
    get url(){
...
describe("Microsites: Multiple Query strings support on filters for Microsite", () => {
    ...
    it("Microsite Consumption:Pass Filters and Search values as a Query String parameter in Microsite URL", ()=>{
        authoring.common.login() 
        //Verify blocks when single filter values are given in a query string URL
        cy.visit(landingPage.url + `?funnelStage=${filterValues.funnelStages}&businessUnit=${filterValues.businessUnits}`)
        ...
        cy.contains(consumption.microsites.cardTitle, target.contents[5]).should("exist") 
        //Validate if the search is filled the value from query string search=value and the contents are filtered based on the search.
        cy.visit(landingPage.url + `?search=${target.contents[2]}`)
        cy.get(consumption.microsites.searchInputLocator).should('have.value',target.contents[2])
        //When filter values doesn't matches with contents available 
        cy.visit(landingPage.url + `?language=${filterValues.language}`)
